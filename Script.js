// ---- App state ----
  ...
  function goToScreen(name){
    ...
    if(name === 'track' && typeof window.trackMapInvalidate === 'function'){
      setTimeout(window.trackMapInvalidate, 150);
    }
  }
  ...
  if(payNowBtn){
    payNowBtn.addEventListener('click', ()=>{
      ...
      if(method === 'wallet'){
        const totalRupees = Math.round(getOrderTotalPaise() / 100);
        if(totalRupees > wallet.balance){
          showToast('Insufficient wallet balance — add money or choose another method');
          return;
        }
        debitWallet(totalRupees, 'Order payment');
        placeOrder('wallet', 'WALLET_' + Date.now());
        return;
      }
  ...
  function placeOrder(method, paymentRef){
    ...
    pushNotification({
      ...
      sub: 'Your fresh order is confirmed and will arrive in about 40 minutes.',
      kind: 'order',
      popup: true,
      actionLabel: 'Track Order'
    });

    // Demo: prime the live-tracking screen for this order. Real integration:
    // call startLiveTracking() from a socket "order:outForDelivery" event
    // with the real rider name + starting coordinates instead.
    startLiveTracking({ etaMinutes: 40, riderName: 'Ravi Kumar' });
    ...
    function tick(){
      ...
      if(minsLeft <= 0 && !etaSoundPlayed){
        ...
        pushNotification({
          ...
          sub: 'The delivery partner is at your doorstep with your fresh order.',
          kind: 'deliver',
          popup: true,
          actionLabel: 'View Order'
        });
        if(typeof markTrackingDelivered === 'function') markTrackingDelivered();
      }
    }
  ...
  function pushNotification(n){
    ...
    renderNotifBadge();
    // Only pop the loud center-screen alert for LIVE events (not the
    // silent history seed on first load) — callers pass n.popup:true.
    if(n.popup){
      showAlertPopup(n);
    }
  }
  ...
  /* =========================================================
     POPUP ALERT CARD + LOUD SOUND
     Fires a center-screen animated card (with a high-pitched alert
     tone) for order / delivery / renewal events. Wire real triggers
     from backend socket events later — e.g.:
       socket.on('order:outForDelivery', d => pushNotification({
         title:'Order out for delivery', sub:`${d.rider} is on the way`,
         kind:'order', popup:true
       }));
  ========================================================= */
  let alertAudioCtx = null;

  // Loud, attention-grabbing two-tone chime (works without any audio file).
  function playAlertSound(){
    try{
      ...
      function tone(freq, start, dur, peakGain){
        ...
        g.gain.exponentialRampToValueAtTime(peakGain, now + start + 0.03);
        g.gain.exponentialRampToValueAtTime(0.0001, now + start + dur);
        o.start(now + start);
        o.stop(now + start + dur + 0.02);
      }
      // Bright high-pitched two-note "ding-dong" style alert, loud peak gain.
      tone(1568, 0.00, 0.28, 0.35);   // G6 — high, sharp
      tone(1976, 0.16, 0.34, 0.32);   // B6 — higher, rings out
  ...
  function showAlertPopup(n){
    alertPopupQueue.push(n);
    if(!alertPopupShowing) renderNextAlertPopup();
  }

  function renderNextAlertPopup(){
    const backdrop = document.getElementById('alertPopBackdrop');
    if(!backdrop || alertPopupQueue.length === 0){ alertPopupShowing = false; return; }
    alertPopupShowing = true;
    const n = alertPopupQueue.shift();

    document.getElementById('alertPopIcon').innerHTML = iconFor(n.kind);
    document.getElementById('alertPopTitle').textContent = n.title;
    document.getElementById('alertPopSub').textContent = n.sub || '';

    const actionBtn = document.getElementById('alertPopAction');
    actionBtn.textContent = n.actionLabel || (n.kind === 'order' ? 'Track Order' : 'View');
    actionBtn.onclick = ()=>{
      closeAlertPopup();
      if(typeof n.onAction === 'function'){ n.onAction(); }
      else if(n.kind === 'order' || n.kind === 'deliver'){ goToScreen('track'); }
      else { openNotifPanel(); }
    };
  ...
  function closeAlertPopup(){
    ...
    setTimeout(renderNextAlertPopup, 180);
  }

  document.getElementById('alertPopClose').addEventListener('click', closeAlertPopup);
  document.getElementById('alertPopDismiss').addEventListener('click', closeAlertPopup);
  document.getElementById('alertPopBackdrop').addEventListener('click', (e)=>{
    if(e.target.id === 'alertPopBackdrop') closeAlertPopup();
  });

  window.showAlertPopup = showAlertPopup;

  /* =========================================================
     WALLET (Pakka Cash) — in-memory mock, ready for a backend swap.
     Real integration points:
       GET  /api/wallet            -> { balance, transactions[] }
       POST /api/wallet/add-money  -> { orderId } then confirm via
                                       payment gateway same as checkout
       POST /api/wallet/debit      -> called from placeOrder() when
                                       payment method === 'wallet'
  ========================================================= */
  const WALLET_STORAGE_KEY = 'pd_wallet_v1';
  let wallet = {
    balance: 120,
    transactions: [
      { type:'credit', title:'Welcome bonus', amount:100, time:new Date(Date.now() - 9*86400000) },
      { type:'credit', title:'Referral reward', amount:20, time:new Date(Date.now() - 5*86400000) }
    ]
  };

  function saveWallet(){
    try{ localStorage.setItem(WALLET_STORAGE_KEY, JSON.stringify(wallet)); }catch(e){}
  }
  function loadWallet(){
    try{
      ...
      if(saved && typeof saved.balance === 'number'){
        saved.transactions = (saved.transactions||[]).map(t=>({...t, time:new Date(t.time)}));
        wallet = saved;
      }
    }catch(e){}
  }
  ...
  function walletTxnIcon(type){
    if(type === 'credit') return '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--green-dim)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg>';
    return '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--danger)" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12l7 7 7-7"/></svg>';
  }

  function renderWallet(){
    const amtEl = document.getElementById('walletBalanceAmt');
    if(amtEl) amtEl.textContent = wallet.balance.toLocaleString('en-IN');
    const subEl = document.getElementById('accountWalletSub');
    if(subEl) subEl.textContent = '₹' + wallet.balance.toLocaleString('en-IN') + ' available';
    const payWalletSub = document.querySelector('.pay-method[data-method="wallet"] .pm-sub');
    if(payWalletSub) payWalletSub.textContent = 'Pakka Cash · ₹' + wallet.balance.toLocaleString('en-IN') + ' available';
    ...
    wallet.transactions.slice().sort((a,b)=> b.time - a.time).forEach(t=>{
      const row = document.createElement('div');
      row.className = 'wallet-txn';
      row.innerHTML = `
        <div class="wallet-txn-ic ${t.type}">${walletTxnIcon(t.type)}</div>
        <div class="wallet-txn-body">
          <div class="wallet-txn-title">${t.title}</div>
          <div class="wallet-txn-time">${timeAgo(t.time)}</div>
        </div>
        <div class="wallet-txn-amt ${t.type}">${t.type === 'credit' ? '+' : '-'}₹${t.amount.toLocaleString('en-IN')}</div>`;
      list.appendChild(row);
    });
  }

  function creditWallet(amount, title){
    wallet.balance += amount;
    wallet.transactions.unshift({ type:'credit', title: title || 'Money added', amount, time: new Date() });
    saveWallet();
    renderWallet();
  }
  function debitWallet(amount, title){
    if(amount > wallet.balance) return false;
    wallet.balance -= amount;
    wallet.transactions.unshift({ type:'debit', title: title || 'Order payment', amount, time: new Date() });
    saveWallet();
    renderWallet();
    return true;
  }
  ...
  document.getElementById('walletAddMoneyBtn').addEventListener('click', ()=>{
    addMoneyInput.value = '';
    document.querySelectorAll('#addMoneyBackdrop .wallet-chip').forEach(c=>c.classList.remove('selected'));
    addMoneyBackdrop.classList.add('show');
  });
  document.getElementById('addMoneyCloseBtn').addEventListener('click', ()=> addMoneyBackdrop.classList.remove('show'));
  addMoneyBackdrop.addEventListener('click', (e)=>{ if(e.target.id === 'addMoneyBackdrop') addMoneyBackdrop.classList.remove('show'); });

  document.querySelectorAll('#addMoneyBackdrop .wallet-chip').forEach(chip=>{
    chip.addEventListener('click', ()=>{
      document.querySelectorAll('#addMoneyBackdrop .wallet-chip').forEach(c=>c.classList.remove('selected'));
      chip.classList.add('selected');
      addMoneyInput.value = chip.dataset.amt;
    });
  });

  document.getElementById('addMoneyConfirmBtn').addEventListener('click', ()=>{
    const amt = parseInt(addMoneyInput.value, 10);
    if(!amt || amt <= 0){ showToast('Enter a valid amount'); return; }
    // Real integration: run this through the same Razorpay flow as
    // checkout, then call creditWallet() only after payment verification.
    creditWallet(amt, 'Added via UPI (demo)');
    addMoneyBackdrop.classList.remove('show');
    showToast('₹' + amt + ' added to your wallet');
    pushNotification({
      title: 'Money added to wallet',
      sub: '₹' + amt + ' has been credited to your Pakka Cash wallet.',
      kind: 'deliver',
      popup: true,
      actionLabel: 'View Wallet',
      onAction: ()=> goToScreen('wallet')
    });
  });

  // Quick-add chips on the wallet screen itself jump straight to the sheet, prefilled
  document.querySelectorAll('.wallet-hero + .wallet-quick-add .wallet-chip, #screen-wallet .wallet-quick-add .wallet-chip').forEach(chip=>{
    chip.addEventListener('click', ()=>{
      addMoneyInput.value = chip.dataset.amt;
      document.querySelectorAll('#addMoneyBackdrop .wallet-chip').forEach(c=>c.classList.remove('selected'));
      const match = document.querySelector('#addMoneyBackdrop .wallet-chip[data-amt="' + chip.dataset.amt + '"]');
  ...
  /* =========================================================
     LIVE ORDER TRACKING (Swiggy-style rider map)
     DEMO MODE: the rider marker walks along a simulated route toward
     the customer's saved location with a live-updating ETA. This is
     structured so a real backend can drop straight in:

       Real integration (swap the demo interval below):
         const socket = io('/tracking');
         socket.on('rider:location', ({ lat, lng, etaMinutes }) => {
           updateTrackingPosition(lat, lng, etaMinutes);
         });
       Or poll: setInterval(() => fetch('/api/orders/:id/track')
         .then(r=>r.json()).then(d => updateTrackingPosition(d.lat, d.lng, d.etaMinutes)), 5000);
  ========================================================= */
  let trackMap = null, trackRiderMarker = null, trackHomeMarker = null, trackRouteLine = null;
  let trackDemoInterval = null;
  let trackRouteCoords = [];
  let trackRouteIdx = 0;

  function fmtTime(d){ return d.toLocaleTimeString('en-IN', {hour:'numeric', minute:'2-digit'}); }

  function initTrackMap(startLat, startLng, homeLat, homeLng){
    const el = document.getElementById('trackMap');
    if(!el) return;
    if(!trackMap){
      trackMap = L.map(el, { zoomControl:false, attributionControl:true, dragging:true, scrollWheelZoom:false });
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      ...
      const riderIcon = L.divIcon({
        ...
        html:'<div style="width:34px;height:34px;border-radius:50%;background:#E8A400;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.3);border:2px solid #fff;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="18.5" cy="17.5" r="3.5"/><path d="M15 6a1 1 0 0 0-1 1v4l4 4h1.5"/><path d="M2 17.5h3.5M9 4h4l2 6.5"/></svg></div>'
      });
      const homeIcon = L.divIcon({
        className:'', iconSize:[28,28], iconAnchor:[14,28],
        html:'<div style="width:28px;height:28px;border-radius:50% 50% 50% 0;background:#4CAF6D;transform:rotate(-45deg);box-shadow:0 2px 6px rgba(0,0,0,.3);"></div>'
      });

      trackRiderMarker = L.marker([startLat, startLng], { icon: riderIcon }).addTo(trackMap);
      trackHomeMarker = L.marker([homeLat, homeLng], { icon: homeIcon }).addTo(trackMap);
      trackRouteLine = L.polyline([[startLat,startLng],[homeLat,homeLng]], { color:'#E8A400', weight:3, opacity:.65, dashArray:'6,7' }).addTo(trackMap);
    } else {
      trackRiderMarker.setLatLng([startLat, startLng]);
      trackHomeMarker.setLatLng([homeLat, homeLng]);
      trackRouteLine.setLatLngs([[startLat,startLng],[homeLat,homeLng]]);
    }
    trackMap.fitBounds(trackRouteLine.getBounds(), { padding:[36,36] });
    setTimeout(()=> trackMap.invalidateSize(), 200);
  }

  function updateTrackingPosition(lat, lng, etaMinutes){
    if(trackRiderMarker) trackRiderMarker.setLatLng([lat, lng]);
    const etaEl = document.getElementById('trackEtaMins');
    if(etaEl) etaEl.textContent = Math.max(0, Math.round(etaMinutes));
  }
  window.updateTrackingPosition = updateTrackingPosition;

  function markTrackingDelivered(){
    document.getElementById('trackStepOut').classList.remove('active');
    document.getElementById('trackStepOut').classList.add('done');
    const deliveredStep = document.getElementById('trackStepDelivered');
    deliveredStep.classList.add('done');
    deliveredStep.querySelector('.track-step-time').textContent = fmtTime(new Date());
    const etaEl = document.getElementById('trackEtaMins');
    if(etaEl) etaEl.textContent = '0';
    const label = document.querySelector('.track-eta-label');
    if(label) label.textContent = 'Delivered';
    if(trackDemoInterval){ clearInterval(trackDemoInterval); trackDemoInterval = null; }
  }
  window.markTrackingDelivered = markTrackingDelivered;

  function startLiveTracking(opts){
    opts = opts || {};
    const etaMinutes = opts.etaMinutes || 20;
    const riderName = opts.riderName || 'Ravi Kumar';

    // Demo coordinates: Kondapur, Hyderabad area (matches the app's
    // default fallback location used elsewhere in this file).
    const homeLat = 17.4615, homeLng = 78.3672;
    const startLat = homeLat + 0.028, startLng = homeLng + 0.031; // rider ~3km away

    document.getElementById('trackRiderName').textContent = riderName;
    document.getElementById('trackEtaMins').textContent = etaMinutes;
    document.querySelector('.track-eta-label').textContent = 'Arriving soon';
    document.getElementById('trackTimeConfirmed').textContent = fmtTime(new Date());
    document.getElementById('trackTimePacked').textContent = fmtTime(new Date());
    document.getElementById('trackTimeOut').textContent = fmtTime(new Date());
    document.getElementById('trackStepOut').classList.remove('done');
    document.getElementById('trackStepOut').classList.add('active');
    document.getElementById('trackStepDelivered').classList.remove('done');

    initTrackMap(startLat, startLng, homeLat, homeLng);

    // Build a simple straight-line route with slight jitter to feel organic.
    trackRouteCoords = [];
    const steps = 60;
    for(let i=0;i<=steps;i++){
      const f = i/steps;
      const jitter = Math.sin(f*Math.PI*3) * 0.0009;
      trackRouteCoords.push([
        startLat + (homeLat-startLat)*f + jitter,
        startLng + (homeLng-startLng)*f + jitter
      ]);
    }
    trackRouteIdx = 0;

    if(trackDemoInterval) clearInterval(trackDemoInterval);
    const tickMs = Math.max(800, (etaMinutes*60*1000) / steps);
    trackDemoInterval = setInterval(()=>{
      trackRouteIdx++;
      if(trackRouteIdx >= trackRouteCoords.length){
        clearInterval(trackDemoInterval);
        trackDemoInterval = null;
        return;
      }
      const [lat,lng] = trackRouteCoords[trackRouteIdx];
      const remainingMins = Math.max(0, Math.round(etaMinutes * (1 - trackRouteIdx/steps)));
      updateTrackingPosition(lat, lng, remainingMins);
      if(remainingMins <= 3){
        document.querySelector('.track-eta-label').textContent = 'Almost there';
      }
    }, tickMs);
  }
  window.startLiveTracking = startLiveTracking;
  window.trackMapInvalidate = function(){ if(trackMap) trackMap.invalidateSize(); };

  // If the user opens Track Order before ever placing one, show a
  // reasonable demo state instead of a blank screen.
  document.querySelectorAll('[data-goto="track"]').forEach(el=>{
    el.addEventListener('click', ()=>{
      if(!trackMap && !trackDemoInterval){
        startLiveTracking({ etaMinutes: 18, riderName: 'Ravi Kumar' });
      }
    });
  });
  ...
  (function(){
    ...
    function maybeShowRenewalReminder(){
      ...
      if(daysToRenew <= 3 && daysToRenew >= 0){
        if(!renewalPopupShown){
          renewalPopupShown = true;
          pushNotification({
            title: 'Your plan renews in ' + daysToRenew + ' day' + (daysToRenew === 1 ? '' : 's'),
            sub: 'Family Pack · Monthly is about to renew. Review or confirm it now.',
            kind: 'renew',
            popup: true,
            actionLabel: 'Review Plan',
            onAction: ()=> document.getElementById('renewBackdrop').classList.add('show')
          });
        } else {
